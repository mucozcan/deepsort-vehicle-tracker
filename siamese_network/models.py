...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, output1, output2, label):
        ...
        losses = 0.5 * (label.float() * euclidean_distance + (1 + (-1 * label)).float()
                        * F.relu(self.margin - (euclidean_distance + self.eps).sqrt()).pow(2))
...
class TripletLoss(nn.Module):
    ...
    def forward(self, anchor, positive, negative, size_average=True):
        distance_positive = F.cosine_similarity(
            anchor, positive)  # Each is batch X 512
        distance_negative = F.cosine_similarity(anchor, negative)  # .pow(.5)
        # Margin not used in cosine case.
